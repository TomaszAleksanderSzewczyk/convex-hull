...
export default function AlgorithmControls({
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      <h2 className="text-lg font-semibold mb-3 text-gray-800">Algorithm Visualization</h2>

      {/* Current step info */}
      <div className={`p-3 rounded-lg border mb-4 ${stepTypeColors[currentStep?.type || 'start']}`}>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium uppercase">{currentStep?.type}</span>
          <span className="text-xs">Step {currentStepIndex + 1} / {steps.length}</span>
        </div>
      ...
      {/* Speed control */}
      <div className="mb-4">
        <label className="text-sm text-gray-600 block mb-1">Speed: {speed}x</label>
        <input
          type="range"
          min="0.5"
          max="3"
